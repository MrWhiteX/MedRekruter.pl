...
import getFavourite from 'services/offer/getFavourite';
...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
  ...
  const offers = await getFavourite(session.user.email);
...
export default function MyFavouriteOffers({ offers }: OffersProps) {
  ...
  const emptyArray = (
    <div className=" text-center -m-4">
      ...
      <p className="mt-7">{dictData.favouriteOffersEmptyState}</p>
      <Link href="/offers">
